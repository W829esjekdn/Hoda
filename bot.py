...
import json
API_ID = '20301168'
API_HASH = 'aa0ee71e2898ec3a2f48031b360dccb4'
bot_token = "6760209211:AAFf6lP1E8wnmuc3RmS30VfepGgnuPfsLmg"
running_processes = {}
try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message and update.message.chat.type == "private":
        if (str(update.message.chat.id) == str(info["sudo"])):
            ...
            await update.message.reply_text("مرحبا بك في سورس التجميع الخاص ببوتات 🤑حودا :\n\n- اشترك في قناة تحديثات بوت التجميع :@H_o_d_aa\n\n- سرعة التجميع : " + str(info["sleeptime"]), reply_markup=reply_markup)
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await update.message.reply_text("مرحبا بك في سورس التجميع الخاص ببوت حودا🤑 :\n\n- سرعة التجميع : " + str(info["sleeptime"]), reply_markup=reply_markup)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (query.data == "addecho"):
        ...
        if (str(query.message.chat.id) == str(info["sudo"])):
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("ممرحبا بك في سورس التجميع الخاص ببوتات 🤑 حودا  :\n\n- سرعة التجميع : " + str(info["sleeptime"]), reply_markup=reply_markup)
        elif (str(query.message.chat.id) in info["admins"]):
            keyboard = [
                [
                    InlineKeyboardButton(
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("مرحبا بك في بوت حودا🤑 :\n\n- سرعة التجميع : " + str(info["sleeptime"]), reply_markup=reply_markup)
